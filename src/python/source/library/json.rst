...
.. note:: セパレータ
    ...
    - デフォルトでセパレータにスペースが入っているので注意。
    - 変更するには、separators=('アイテムセパレータ', 'キーセパレータ')を指定する必要があります。
    - https://github.com/python/cpython/blob/master/Lib/json/encoder.py#L103

.. note:: オブジェクトソート

    - デフォルトでオブジェクトをソートしません。
    - オブジェクトのキーでソートするには、 *sort_keys=True* を指定する必要があります。
    - https://github.com/python/cpython/blob/master/Lib/json/encoder.py#L351
