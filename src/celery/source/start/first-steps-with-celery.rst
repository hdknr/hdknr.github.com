...
- Choosing and installing a :term:`message broker`.
...
comes in the form of a separate service called a :term:`message broker`.
...
.. note::
    - :term:`Kombu` は基本的に :term:`amqp` をブローカーのモデルとして考えていて、 
      :term:`ampqlib` と、:term:`Pika` を"Native" トランスポートとして扱っている。
    - それ以外は**Virtual** トランスポート。
...
.. note::
    ...
    - データベースは推奨されませんが、小規模だったらいいのではないでしょうか。
...
If you're already using a Django database for example, 
using it as your :term:`message broker` can be convenient 
while developing even if you use a more robust system in production.
...
you want to create a :ref:`dedicated module <project-layout>`.
...
setup_loader()では CELERY_LOADERをDjangoLoaderに設定します。
