...
Tasks are the building blocks of :term:`Celery applications`.

.. note::
    タスク:Celeryアプリケーションの構成要素

A task can be created out of any :term:`callable` and defines what happens
when the :term:`worker` receives a particular :term:`message`.

.. note::
    - callable とは独立して作成する事ができる
    - WorkerがMessageを受け取った時に、何が起きるかを定義する。 

name
-----

Every task has unique name which is referenced in the :term:`message`,
so that the :term:`worker` can find the right task to execute.

.. note::
    - **name** 属性(ユニーク)をもっていて、これでMessageを参照できる。
    - なのでWorkerは正しいタスクを実行する事が可能

idempotent
------------------

It's not a requirement, but it's a good idea to keep your tasks
:term:`idempotent`.  Idempotence means that a task can be applied multiple
times without changing the result.

.. note::
    - Taskは何度実行されても結果は同じになるようにしておくべき

This is important because the task message will not disappear
until the message has been :term:`acknowledged`. A worker can reserve
...
.. note::
    - Messageが受領されるまでTaskは消えない
    - Workerは多くのメッセージを保存しておくので、仮にWorkerが死んでも別のMessageが別のWorkerに届けられる
    - つまり、同じメッセージは全システム無いに複数存在する事があるので、 :term:`idempotent` が重要。 
...
.. note::
    - WorkerはTaskが :term:`idempotent` かどうか知らない
    - よって、デフォルトの動作は実行される前に事前に :term:`acknowledged` される。
    - そうすれば、開始済のTaskが複数回実行されることはない。
...
See also the FAQ entry :ref:`celery:faq-acks_late-vs-retry`.

.. note::
    - Taskが :term:`idempotent` であれば、 :attr:`acks_late` オプションを使うと良い
    - Taskが実行 **された後** (デフォルトは「される前」) に :term:`acknowledged` されるようになる。

Basics
======

.. note::
    - 最も簡単にTaskを咲く背するには、@clery.task でコレータを使う
    - @celery.task(serializer='json')とかTaskの設定が可能
    - Celeryインスタンスのtask()を使うが、Djangoの場合は celeryモジュールからimportする(古いやり方)

You can easily create a task from any :term:`callable` by using
...
.. note::
    複数のデコレータをつかう callable の場合、 @Celery.task を最後に評価されるようにする。
...
.. note::
    - name='なまえ' で Taksに名前を付ける事ができます
    - デフォルトでは :ref:`task-naming-relative-imports` のルールに従います。 
    - キューでバッティングしないようにモジュールパスをネームスペースするとよいです。 
    - callable.name でタスクのNameを参照する事ができます。
...
.. note::
    - import を相対的に行わない方がよい。
    - Workerが見つけられない事がある。
    - 相対的に import するのであれば、name を明示的に指定する事。
...
.. note::
    - 現在のタスクの Context は Task内部で callabe.request で参照可能
    - 現在のタスクは celery.current_task で参照することができる。
...
:is_eager: Set to :const:`True` if the task is executed locally in
           ...
           :term:`Worker` では無く、ローカルプロセスでTaskが実行されるのであればTrue(:term:`Eager`)
           

:logfile: The file the worker logs to.  See `Logging`_.

          :term:`Worker` が記録するファイル。
...
:delivery_info: Additional message delivery information. This is a mapping
                ...
                メッセージ配送の付加情報。
                このTaskを:term:`Deliver` するのに使われた :term:`Exchange` と :term:`Routing Key` を含むマッピング情報。
                Task.retry はこの情報をもとに同じ前回と同じ :term:`destination queue` にTask を再送する。
                この dict に含まれるキーは 使われる :term:`Message Broker` によって違う。
...
.. note::
    - デフォルトログとマニュアルログ
    - "clery.task" を継承したロガーを定義可能。 
    - Pythonの標準loggerライブラリが使われています。
    - :setting:`CELERY_REDIRECT_STDOUTS` で print を記録するかどうか設定可能です(デフォルトで記録します).

The :term:`worker` will automatically set up logging for you, 
or you can configure logging manually.

A special logger is available named "celery.task", 
you can inherit from this logger to automatically get the :ref:`task name <task-names>` 
and :ref:`unique id <task-request-info>` as part of the logs.
...
for which documentation can be found in the :mod:`logging` module.

You can also simply use :func:`print`, 
as anything written to standard out/-err will be redirected to the workers logs 
by default (see :setting:`CELERY_REDIRECT_STDOUTS`).
