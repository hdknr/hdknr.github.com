...
.. glossary::
    ...
    eventlet
        ...
        - Celeryも実験的にeventlet 対応中

            - http://docs.celeryproject.org/en/latest/internals/reference/celery.concurrency.eventlet.html

    gevent
        - gevent is a Python networking library that uses :term:`greenlet` to provide a synchronous API on top of :term:`libevent` event loop.
        - ( http://www.gevent.org/intro.html  )

        - Celeryも実験的にgevent 対応中  

            - http://docs.celeryproject.org/en/latest/internals/reference/celery.concurrency.gevent.html

    greenlet
        - The greenlet package is a spin-off of Stackless, a version of CPython that supports micro-threads called "tasklets". Tasklets run pseudo-concurrently (typically in a single or a few OS-level threads) and are synchronized with data exchanges on "channels".
        - http://pypi.python.org/pypi/greenlet/
        - eventlet, gevent が使ってます。
        - celery.concurrency.base
        - celery.concurrency.eventlet
        - celery.concurrency.gevnet
        - celery.utils.thread
        - celery.worker
